...
productoEntrada = prompt(
  "Ingrese el articulo solicitado o presione ENTER para finalizar"
);
...
while (productoEntrada != "") {
  if (existeArticulo) {
    if ((productoEntrada = articulo[indexArticulo])) {
      alert(
        "El articulo " +
          articulo[indexArticulo] +
          " Con el precio $" +
          precio[indexArticulo] +
          " Mas IVA subtotal $" +
          subTotal(precio[indexArticulo])
      );
      console.log(
        "El articulo " +
          articulo[indexArticulo] +
...
function init() {
  const galeria = document.querySelector(".main_galeria");
  if (galeria) {
    galeria.innerHTML = "";
    for (let producto of productos) {
      ...
      image.setAttribute("src", `./Imagenes/Ofertas/Oferta${+producto.id}.jpg`);
      container.appendChild(btn);
      btn.addEventListener("click", () =>
        alert(
          `Aca tenes que hacer algo con el boton | EL id de producto es ${producto.id} `
        )
      );
    ...
    console.log("No se encontro la galeria loquito");
  }
}
